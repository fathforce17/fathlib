...
<main>
    ...
    <section class="content-section">
        <div class="container px-4 px-lg-5">
                {{-- Check if both productcategories and products are empty --}}
                @if ($productcategories->isEmpty() && $products->isEmpty())
                    <!-- Dummy Data START -->
                ...
                @else
                    ...
                    <ul class="nav justify-content-center pb-3">
                        @foreach($productcategories as $productcategory)
                            <li class="nav-item">
                                <a class="nav-link {{ isset($selectedProductcategoryId) && $productcategory->id == $selectedProductcategoryId ? 'active' : '' }}" href="{{ route('products', ['productcategory' => $productcategory->id]) }}">{{ $productcategory->title }}</a>
                            </li>
                        @endforeach
                    </ul>
                                    ...
                                    <div class="col-lg-4 col-sm-12">
                                        <div class="card w-75 h-75">
                                            <div class="max-w-100 max-h-100">
                                                <a href="{{ url('products/detail/' . $item->uuid) }}" style="text-decoration: none;">
                                                    <img src="{{ App\Models\Media::getimageweb($item->meta['media_token']) }}" class="card-img-top" alt="{{$item->name}}">
                                                </a>
                                            </div>
                                            <div class="card-body" style="flex-grow: 1;">
                                                <a href="{{ url('products/detail/' . $item->uuid) }}" style="text-decoration: none; color: inherit;">
